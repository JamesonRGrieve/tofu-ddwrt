...
func TestRunReconcile(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			restart:  func(string) error { return errors.New("ddwrt ssh: exit 255: connection refused") },
		...
		},
		{
			...
			restart: func(s string) error {
				if s == "dnsmasq" {
					return errors.New("ddwrt ssh: exit 1: unknown service") // service absent on this box
				}
				return nil
			},
