...
// NewObjectDataSource constructs the ddwrt_nvram data source.
...
func (d *nvramDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Read DD-WRT NVRAM. Set `key` to read a single variable (`value` + `present`); " +
...
func (d *nvramDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	if !ok {
		resp.Diagnostics.AddError("Unexpected provider data", fmt.Sprintf("expected *ddwrt.Client, got %T", req.ProviderData))
		return
	}
	d.client = client
}
...
func (d *nvramDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if m.Key.IsNull() || key == "" {
		...
		if err != nil {
			resp.Diagnostics.AddError("DD-WRT nvram show failed", err.Error())
			return
		}
	...
	if err != nil {
		resp.Diagnostics.AddError("DD-WRT nvram get failed", err.Error())
		return
	}
